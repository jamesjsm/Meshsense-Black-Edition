...
intercept(
  ...
  (err) => {
    if (String(err).includes('Possible EventTarget memory leak detected')) return ''
    consoleLog.push(err)
    while (consoleLog.length >= logSize) consoleLog.shift()
    return err
  }
...
createRoutes((app) => {
  app.post('/clearNodeRequestStatus', (req, res) => {
    ...
    if (!Number.isInteger(destination) || destination <= 0 || destination >= broadcastId) return res.sendStatus(400)
    clearCompletedStatuses(destination)
    return res.sendStatus(204)
  })
  app.post('/requestNodeData', async (req, res) => {
    if (!isAuthorized(req)) return res.sendStatus(403)
    try {
      await requestNodeData(req.body.destination, req.body.kind)
  ...
  app.post('/traceRoute', async (req, res) => {
    if (!isAuthorized(req)) return res.sendStatus(403)
    let destination = req.body.destination
    if (!Number.isInteger(destination) || destination <= 0 || destination >= broadcastId || destination === myNodeNum.value) return res.status(400).json({ error: 'Choose a remote node' })
    if (connectionStatus.value !== 'connected') return res.status(409).json({ error: 'Connect to a radio first' })
