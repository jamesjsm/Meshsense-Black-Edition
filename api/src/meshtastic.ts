...
import { nodeRequests, requestKinds, setRequestStatus, receiveNodeData, traceRequests, setTraceStatus, type RequestKind } from './nodeRequests'
import {
  Channel,
  MeshPacket,
  NodeInfo,
...
export async function connect(address?: string) {
  ...
  connection.events.onUserPacket.subscribe((e) => {
    receiveNodeData(e.from, 'nodeInfo')
  ...
  connection.events.onTelemetryPacket.subscribe((e) => {
    ...
    for (const kind of ['deviceMetrics', 'environmentMetrics', 'powerMetrics'] as RequestKind[]) {
      if (telemetry[kind] !== undefined) receiveNodeData(e.from, kind)
    }
  ...
  connection.events.onPositionPacket.subscribe((e) => {
    receiveNodeData(e.from, 'position')
    let { id, data } = copy(e)
    let packet: MeshPacket
  ...
  connection.events.onTraceRoutePacket.subscribe((e) => {
    ...
    let packet: MeshPacket
    if (id) packet = packets.upsert({ id, data })
    if (e.from && data && e.to === myNodeNum.value) {
      setTraceStatus(e.from, 'Route received')
      let node = nodes.upsert({ num: e.from, trace: data, traceOrigin: packet?.to || myNodeNum.value, traceReceivedAt: Date.now() })
      ...
      for (let num of new Set<number>([...(data.route ?? []), ...(data.routeBack ?? [])])) {
        if (!num || num === broadcastId) continue
...
export function traceRoute(destination: number) {
  if (connectionStatus.value !== 'connected' || pendingTraceroutes.value.includes(destination)) return
  const previous = traceRequests.value[destination]
  if (previous?.status === 'Waiting for route' || (previous && Date.now() - previous.at < 60000)) return
  ...
  setTraceStatus(destination, 'Queued')
...
async function processTraceRoutes() {
  ...
  if (connectionStatus.value !== 'connected') {
    for (const num of pendingTraceroutes.value) setTraceStatus(num, 'Disconnected')
    pendingTraceroutes.set([])
    queueProcessing = false
    return
  }
  setTraceStatus(destination, 'Waiting for route')
  const started = traceRequests.value[destination].at
  const timeout = setTimeout(() => {
    if (traceRequests.value[destination]?.at === started && traceRequests.value[destination]?.status === 'Waiting for route') {
      setTraceStatus(destination, 'No route received (timed out)')
    }
  }, 90000)
  ...
  try {
    if (connectionStatus.value === 'connected') await connection.traceRoute(destination)
  } catch (error) {
    clearTimeout(timeout)
    setTraceStatus(destination, 'Send failed')
    console.error('[meshtastic] Traceroute send failed', error)
  }
...
let lastNodeRequest = 0
const lastRequests = new Map<string, number>()
export async function requestNodeData(destination: number, kind: RequestKind) {
  if (!Number.isInteger(destination) || destination <= 0 || destination >= broadcastId || !requestKinds.includes(kind)) {
    throw Object.assign(new Error('Invalid node or request type'), { status: 400 })
  }
  if (connectionStatus.value !== 'connected') throw Object.assign(new Error('Connect to a radio first'), { status: 409 })
  const key = `${destination}:${kind}`
  const now = Date.now()
  if (now - (lastRequests.get(key) ?? 0) < 60000 || now - lastNodeRequest < 5000) {
    throw Object.assign(new Error('Please wait: 60 seconds between identical requests, 5 seconds between other requests'), { status: 429 })
  }
  lastRequests.set(key, now)
  lastNodeRequest = now
  setRequestStatus(destination, kind, 'Waiting for data')
  const timer = setTimeout(() => {
    if (nodeRequests.value[key]?.status === 'Waiting for data' && nodeRequests.value[key]?.at === nowStatus) {
      setRequestStatus(destination, kind, 'No data received (timed out)')
    }
  }, 90000)
  const nowStatus = nodeRequests.value[key].at
  try {
    const channel = nodes.value.find(n => n.num === destination)?.channel ?? 0
    const { payload, port } = requestPayload(kind)
    return await connection.sendPacket(payload, port, destination, channel, true, true)
  } catch (error) {
    clearTimeout(timer)
    setRequestStatus(destination, kind, 'Send failed')
    throw error
  }
}
