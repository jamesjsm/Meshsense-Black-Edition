...
import icon from '../../resources/black-edition.png?asset'
...
const independentBuild = /-(custom|black)\./.test(app.getVersion())
// Keep the settings location used by earlier custom builds after the display-name change.
if (independentBuild) app.setPath('userData', join(app.getPath('appData'), 'meshsense'))
...
app.whenReady().then(async () => {
  ...
  apiProcess = utilityProcess.fork(apiPath, process.argv, {
    stdio: 'pipe',
    env: independentBuild ? { ...process.env, MESHSENSE_DATA_DIR: process.env.MESHSENSE_DATA_DIR || join(app.getPath('userData'), 'radio-data') } : process.env
  })
  ...
  apiProcess.on('message', (e) => {
    if (independentBuild && ['installUpdate', 'checkUpdate', 'setUpdateChannel'].includes(e.event)) {
      apiProcess.postMessage({ event: 'update-not-available', body: { version: app.getVersion() } })
      return
    }
