...
<main class="w-full grid grid-cols-[auto_1fr] gap-2 p-2 overflow-auto h-full">
  <News />
  <div class="app-sidebar flex flex-col gap-2 content-start h-full overflow-auto">
    {#if $hasAccess}
  ...
  <div id="content" class="grid grid-rows-[5fr_3fr] content-start h-full overflow-auto gap-2 relative">
    ...
    {:else}
      <div class="grid items-center px-5 m-auto">
        <div class="text-3xl font-bold text-blue-500">Meshsense Black Edition</div>
        <div class="mt-2 text-white/60">Independent edition · Based on MeshSense by Affirmatech</div>
        <button class="btn mt-2 w-fit" on:click={() => showPage('About / Legal')}>About / Legal</button>
        ...
        <div class="font-normal absolute m-2 top-0 right-0 flex gap-2 items-center">
          <div class="text-xs text-white/50 pr-2 font-bold">Black Edition {$version}</div>
