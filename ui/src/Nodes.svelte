...
<script lang="ts">
  ...
  import NodeActions from './lib/NodeActions.svelte'
  import { routeTarget } from './lib/routes'
      ...
      .sort((a, b) => {
        ...
        switch ($sortField) {
          ...
          case 'batteryVoltage':
            ...
            break
          case 'rssi':
            if (a.hopsAway == 0) aValue = a.rssi ?? 0
            else aValue = $sortDirection === 'asc' ? 999 : -999

            if (b.hopsAway == 0) bValue = b.rssi ?? 0
            else bValue = $sortDirection === 'asc' ? 999 : -999
            break
          case 'snr':
            if (a.hopsAway == 0) aValue = a.snr ?? 0
            else aValue = $sortDirection === 'asc' ? 99 : -99

            if (b.hopsAway == 0) bValue = b.snr ?? 0
    ...
    <div class="p-1 text-sm grid gap-1 overflow-auto h-full content-start">
      {#each $filteredNodes as node (node.num)}
        <div
          ...
          class="node-card rounded px-1 py-0.5 flex flex-col gap-0.5 {node.num == $myNodeNum
            ? 'node-own '
            ...
            <div class="flex gap-1 items-center">
              ...
              {#if node.user?.role != undefined}
                ...
                {:else if node.user.role === 4}
                  <div title="Deprecated Repeater Node (since firmware 2.7.11)" class="bg-red-500/50 text-red-200 rounded px-1 font-bold cursor-help">Re</div>
                ...
                {:else}
                  <div title="Unknown node role: {node.user.role}" class="bg-gray-500/50 rounded px-1 font-bold cursor-help">?{node.user.role}</div>
                {/if}
              {/if}
              {#if node.viaMqtt}
            ...
            <div class="flex gap-1.5 items-center">
              ...
              {#if node.num != $myNodeNum}
                <button
                  ...
                  on:click={() => { routeTarget.set(node.num); axios.post('/traceRoute', { destination: node.num }).catch(e => alert(e.response?.data?.error || 'Traceroute request failed')) }}>↯</button
                >
              {:else if $hasAccess}
