<script lang="ts">
  ...
  let darkMode = (localStorage.getItem('darkMode') ?? 'true') == 'true'
  ...
  export function plotLines(layerName: string, data: number[][][], color = '#FF871D', dashed = false, uncertain = false, overview = false) {
    if (layers[layerName]) map.removeLayer(layers[layerName])
    ...
    layers[layerName] = new VectorLayer({
      ...
      style: (feature) => {
        ...
        const width = overview ? 2 : uncertain ? 3 : dashed ? 4 : 5
        const lineDash = uncertain ? [2, 8] : dashed ? [8, 8] : undefined
        const styles = [
          new Style({ stroke: new Stroke({ color: 'rgba(0,0,0,0.45)', width: width + 2, lineDash }) }),
          new Style({ stroke: new Stroke({ color, width, lineDash }) })
        ]
        const points = geometry.getCoordinates()
        for (let i = 1; !overview && i < points.length; i++) {
          const [a, b] = [points[i - 1], points[i]]
          const fraction = dashed ? 0.65 : 0.45
          const pa = map.getPixelFromCoordinate(a), pb = map.getPixelFromCoordinate(b)
          if (!pa || !pb) continue
          styles.push(new Style({
            geometry: new Point([a[0] + (b[0] - a[0]) * fraction, a[1] + (b[1] - a[1]) * fraction]),
            text: new Text({ text: '➤', font: 'bold 20px sans-serif', rotation: Math.atan2(pb[1] - pa[1], pb[0] - pa[0]), fill: new Fill({ color }), stroke: new Stroke({ color: '#111827', width: 2 }) })
          }))
        }
        return styles
      },
  ...
  export function fitCoordinates(coordinates: number[][]) {
    if (!map || !coordinates.length) return
    map.updateSize()
    map.getView().fit(new LineString(coordinates).getExtent(), { padding: [45, 45, 45, 45], maxZoom: 14, duration: 250 })
  }
  ...
  onMount(() => {
    ...
    return () => { observer.disconnect(); map.setTarget(undefined) }
  })
</script>

<div bind:this={mapElement} class="h-full min-h-0" />
