...
export function getCoordinates(node: NodeInfo | number) {
  ...
  if (node?.position?.longitudeI === undefined || node?.position?.latitudeI === undefined) {
    const approximate = node?.approximatePosition || undefined
    return [approximate?.longitude, approximate?.latitude]
  }
